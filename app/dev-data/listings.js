const listings = [
  {
    id: 1,
    address: "8601 West Shah wali ullah nagar sec 11- 1/2 ",
    bathrooms: 1,
    bedrooms: 2,
    title: " Beautiful Stradford Condo",
    offer: true,
    furnished: true,
    parking: false,
    price: 2000,
    category: "sell",
    images: [
      "https://media.istockphoto.com/photos/large-house-with-steep-roof-and-side-entry-three-car-garage-picture-id1272163106?b=1&k=20&m=1272163106&s=170667a&w=0&h=pTjbBRKTcnhs-oIGpuSB4TyAE5768gp0BefDXxyUNTU=",
    ],
  },
  {
    id: 2,
    address: "Banaras Muhalla Street 9 sec - 14 ",
    bathrooms: 1,
    bedrooms: 4,
    title: " Adventures is Your Friend",
    offer: true,
    furnished: true,
    parking: true,
    price: 1000,
    category: "rent",
    images: [
      "https://media.istockphoto.com/photos/colonial-style-house-picture-id1284097677?b=1&k=20&m=1284097677&s=170667a&w=0&h=1A7BkHG5OU4WCN7m22OOhvVmU21q4UsYVJPrS1kgcKI=",
    ],
  },
  {
    id: 3,
    address: "North Nazimabad Karachi Zone 4th",
    bathrooms: 4,
    bedrooms: 3,
    title: "Very Comfortable for Big Family",
    offer: true,
    furnished: true,
    parking: true,
    price: 250000,
    category: "sell",
    images: [
      "https://media.istockphoto.com/photos/front-view-of-an-acadian-renovated-home-with-columns-sidewalks-and-a-picture-id1332179910?b=1&k=20&m=1332179910&s=170667a&w=0&h=o3j8Y3Cs5iy_U_z43PI7kJZwcDyoguxjscsEFyI5mfw=",
    ],
  },
  {
    id: 4,
    address: "Kolachi house no 687 main shahrha muhallah ",
    bathrooms: 4,
    bedrooms: 3,
    title: "Very Comfortable for Big Family",
    offer: false,
    furnished: true,
    parking: true,
    price: 20000,
    category: "rent",
    images: [
      "https://images.unsplash.com/photo-1568605114967-8130f3a36994?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NHx8aG91c2V8ZW58MHx8MHx8&auto=format&fit=crop&w=500&q=60",
    ],
  },
];

export default listings;
